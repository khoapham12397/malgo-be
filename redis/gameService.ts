  ...
  const quizList = [
    {
      ...
      quizData: {
        description:
          "Với số nguyên dương N ta tính tích các chữ số của nó, tổng các chữ số của nó rồi cộng hai kết quả lại. Tổng cuối cùng ký hiệu là S(N ). Hỏi trong các số nguyên từ 1 đến 100 (kể cả 1 và 100) có bao nhiêu số N thỏa mãn S(N ) = N ?",
        result: 3,
        options: [1, 2, 3, 9],
      },
      quizType: "option",
...
export const getOneQuizState = async (roomId: string, quizKey: string) => {
  try {
    const quizState = await redisClient.HGET(
      `match:${roomId}`,
      `quiz:${quizKey}`
    );
    if (quizState) {
...
export const setOneQuizState = async (
  roomId: string,
  quizKey: string,
  qState: { state: number; socketId: string | null }
) => {
  try {
...
export const addOneSolve = async (
  roomId: string,
  quizKey: string,
  socketId: string
) => {
  try {
    const [qState, sInfo, qSolved, roundId, usrs] = await redisClient.HMGET(
      `match:${roomId}`,
      [
        `quiz:${quizKey}`,
        `socket:${socketId}`,
    ...
    if (qState && sInfo && qSolved) {
      ...
      const newState = {
        state:
          quizSolved.length < Math.min(Math.floor(users.length / 2), 2)
            ? QUIZ_STATE.OPENING
            : QUIZ_STATE.CLOSED,
        socketId: null,
      };
      ...
      Object.keys(match).forEach((key) => {
        ...
        if (key.substring(0, 11) == "quiz:solved") {
          matchState.quizSolveds[key.substring(12, key.length)] = JSON.parse(
            match[key]
          );
        } else if (key.substring(0, 4) == "quiz") {
          matchState.quizStates[key.substring(5, key.length)] = JSON.parse(
            match[key]
          );
        }
      });
...
export const getOneQuiz_SocketInfo_Round = async (
  roomId: string,
  quizKey: string,
  socketId: string
) => {
  try {
    const [quizState, quizSolved, socketInfo, roundId] =
      await redisClient.HMGET(`match:${roomId}`, [
        `quiz:${quizKey}`,
        `quiz:solved:${quizKey}`,
        `socket:${socketId}`,
        "roundId",
    ...
    if (quizState && quizSolved && socketInfo) {
      return [
        JSON.parse(quizState),
        JSON.parse(quizSolved),
...
export const enterQuiz = async (
  roomId: string,
  quizKey: string,
  socketId: string
) => {
  try {
    let qState: any = await redisClient.HGET(
      `match:${roomId}`,
      `quiz:${quizKey}`
    );
...
export const leaveQuiz = async (
  ...
  try {
    let [qState, roundId]: any = await redisClient.HMGET(`match:${roomId}`, [
      `quiz:${quizKey}`,
      "roundId",
    ]);
    let ans = Number(
      await redisClient.HGET(`round:${roundId}:quiz:result`, quizKey)
    );
    ...
    if (qState) {
      ...
      if (qState["state"] == QUIZ_STATE.KEEPING) {
        const newState = JSON.stringify({
          state: successed ? QUIZ_STATE.CLOSED : QUIZ_STATE.OPENING,
          socketId: successed ? socketId : null,
        });
        if (successed) {
          let socketState: any = await redisClient.HGET(
            `match:${roomId}`,
            `socket:${socketId}`
          );
          if (socketState) {
