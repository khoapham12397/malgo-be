...
import { InsertChatMsgGroup, InsertChatMsgP2P, InsertPostMsg, ReferenceMessage } from './controllers/chatController';
...
const startGameRoom = async (waitingSockets: WatingSockets, roundId:string)=>{
    ...
    const socketList = waitingSockets.sockets.map(item => ({
        socketId: item.socket.id, username: item.socketInfo.username
        }));
...
const leaveGameRoom = (socket: Socket)=>{
    ...
    if(roomId) {
        ...
        if(room && room.size == 1) {
            ...
            //console.log('server emit leave-game');
...
export const initIOSocket = (server: any) =>{
    ...
    io.on("connection", (socket: Socket)=>{
        ...
        socket.on('game-req', (data: any) =>  {
            ...
            console.log(`game req from socketID: ${socket.id}`);
            ...
            console.log('current waiting list size: '+ waitingSockets?.sockets.length);
            ...
            if(waitingSockets ){
                ...
                waitingSockets.sockets.push({socket: socket, socketInfo: {username: data.username}});
                
                socket.data.gameState = GAME_STATE.WAITING;

                if(waitingSockets.sockets.length >= maxClientRoom || 
                    ...
                    .catch(error=>{
                        ...
                        console.log(error);
                        //waitingSocket.emit("error-connect", error);
                    })        
            ...
            if(data.roomId == room){
                if(data.type == 'enter') {
                    ...
                    .then(result=>{
                        ...
                        io.to(data.roomId).emit('quiz-state', {quizStateList : lst, matchState: result.matchState});                          
                    })
                    .catch(error=>{
                        io.to(data.roomId).emit('quiz-state',{quizStateList: []});  
                    })
        ...
        socket.on('disconnecting', ()=>{
            ...
            if(room && room.size == 1) {
                ...
                io.to(roomId).emit('leave-game', {socketId: socket.id}); 
            }
            socket.leave(roomId);
        ...
        socket.on("disconnnect-user", ()=>{
            console.log(`user: ${socket.data.username} disconnect socket: ${socket.id}` );
            removeUserSocket(socket.data.username, socket.id);
        });
