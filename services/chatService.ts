...
export const getGroupMessage = async(groupId: string,username:string, params: GetMessageParam)=>{
    try {   
        ...
        if(!group || !group.generalChatSessionId) return { 
        ...
        return {
            messages: messages, sessionId: generateMessageId,
        };
    }catch(error){
        throw error;
    }
...
export const setSeenMsgSessionP2P = async (username: string, sessionId: string)=>{
    try{
...
export const getChatSessionP2P=async(username: string, sessionId: string)=>{
    try {        
...
export const getChatSessionP2PByUsername = (username1: string, username2: string) =>{
    try{    
