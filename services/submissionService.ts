...
import {
  ...
  insertPendingSubmissionV2,
  InsertPendingSubParam,
  insertPendingTokenDirectly,
  insertSingleProblemSubmited,
  insertSubmission,
  insertSubmissionContest,
} from "../redis/submissionService";
import { generator } from "../utils/genId";
import {
  ChildSubmissionBatchParam,
  createChildSubmissionBatch,
  PostSubContestParam,
...
export const createSubmissionDBV2 = async (params: SubmissionParam) => {
  try {
    const subTestNumber = await getNumberSubTest(
      getTestCaseID(params.problemId)
    );
    ...
    insertSingleProblemSubmited(params.username, params.problemId)
      .then((result) => {
        if (result === 1) {
          // TODO :add event to recommeder system
        }
      })
      .catch((error) => {
        console.log(error);
...
export const updateSubmissonDBStatus = async (
  submissionId: string,
  statusArr: Array<StatusUpdate>
) => {
  try {
    ...
    statusArr.forEach((item) => {
      ...
      if (item.statusId > 3) checkResult = item.statusId;
    });
    ...
    if (nextTokens.length > 0 && checkResult < 3) {
      //await redisClient.LPUSH(getPendingTokenQueueKey(), JSON.stringify([submissionId, nextTokens]));
      await insertPendingTokenDirectly(
        JSON.stringify([submissionId, nextTokens])
      );
    } else {
      if (contestId) {
...
export const createSubmissionContest = async (params: SubmissionParam) => {
  try {
    ...
    const submissionId = await insertSubmissionContest(
      params,
      testcase.subTestNumber
    );
...
export const getSubmissionsProblem = async (
  page: number,
  problemId: string,
  username: string | undefined
) => {
...
export const getAllProblemUserSubmited = async () => {
  try {
    const submissions = await prisma.submission.findMany({
      select: {
        problemId: true,
...
export const createSubmissionSimple = async () => {
  try {
  } catch (error) {
    throw error;
  }
