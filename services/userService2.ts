...
export const getFriendList = async (username: string)=>{
    ...
    try{
        ...
        for(let i=0;i<frdShipList.length;i++) {
            let online = false, name = username;
            ...
            lst.push({
                username: name, isOnline: online == true
            })
    ...
    try{
        ...
        const chatSession = await prisma.chatSession.create({
            data: {
                ...
                lastMessage: {message: 'You are connected', type:'system', authorId:'system'} as Prisma.JsonObject,
