import  { addOneSolve, getOneQuizResult, getOneQuizState, getOneQuiz_SocketInfo_Round, getQuizList, getQuizResultList, insertMatch, QUIZ_STATE, setOneQuizState } from "../redis/gameService";
export const initRoom = async (roomId: string, roundId: string, socketList: Array<any>) =>{
    try{
...
export const joinQuiz = async (roomId: string, quizKey: string, socketId: string) =>{
    try {
...
export const leaveQuiz = async (roomId: string, quizKey: string,socketId:string ,result: number | null) => {
    try {
        ...
        let [quizState, quizSolved,socketInfo,roundId] = await getOneQuiz_SocketInfo_Round(roomId, quizKey,socketId);
...
export const initMatchState = (param: InitMatchParam)=>{
    ...
    let quizStates :any = {}, quizSolveds: any = {};
