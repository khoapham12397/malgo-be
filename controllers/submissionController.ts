...
export const createSubmissionContestCtl = async (req: express.Request,res: express.Response)=>{
    try{
        /*
        const username = getUsername(req, '');
        if(!username) {
        ...
        */
        const submissonId = await createSubmissionContest(req.body as SubmissionParam);// base64 encode src;
        return res.status(201).json({
            successed: true, data: {submissionId: submissonId}
        }).end();
        
    }catch(error: any){
        return res.status(error.statusCode).json({successed: false, message: error.message});
    }
}


export const getSubmissionStatusCtl = async (req: express.Request,res: express.Response) =>{
    try{
...
export const getSubmissionsCtl = async (req: express.Request,res: express.Response) =>{
    try{
        const page = Number(req.params.page);
        let username = req.query.username; 
        
        const result = await getSubmissionList(page, username as (string | undefined));
...
export const getSubmissionsProblemCtl = async (req: express.Request,res: express.Response) =>{
    try{
        const page = Number(req.params.page);
        const problemId = req.params.problemId;
        let username = req.query.username; 
        const result = await getSubmissionsProblem(page,problemId,username as (string | undefined));
        return res.status(200).json({
            successed: true, data : result,
        });
    }catch(error :any){
