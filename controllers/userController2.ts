...
import { acceptFriendReq, addUserToGroup, checkFriendShip, checkRelTwoUser, createFriendReq, createGroup, createPostGroup, findUserByEmail, getFriendList, getFriendReqFrom, getFriendReqTo, getGroupList, getGroupMember, getGroupPostList, getShareResource, lookedShare, shareResource } from "../services/userService2";
...
export const getFriendListCtl = async (req: Request, res: Response) =>{
    try{
        ...
        if(username && username == req.params.username) {
            ...
            res.status(200).json({successed: true, data: {
                friendList: friendList
            }})
        }
        else res.status(400).json({successed: false, message: "user not found"});

    }catch(error : any){
        res.status(error.statusCode?error.statusCode:400).json({successed: false, message: error.message});
    }
}
...
export const postFriendReqCtl = async(req: Request, res: Response) =>{
    try{
        ...
        return res.status(error.statusCode?error.statusCode:400).json({successed: false, message: error.message}).end();
    }
}
...
export const checkFriendShipCtl = async (req: Request, res: Response) =>{
    try{
        ...
        if(username && username == req.query.username1) {
            const isFriend = await checkFriendShip(username, req.query.username2 as string);
            return res.status(200).json({successed: true, data: {isFriend: isFriend}});
        }
        else throw new CustomAPIError("You don't have permission",403);
    }catch(error:any){
        console.log(error);
...
export const getRelationShipTwoUserCtl = async (req: Request, res: Response)=>{
    try{
        const username = getUsername(req, req.query.username1 as string);
        if(username && username == req.query.username1) {
            const rel = await checkRelTwoUser(username, req.query.username2 as string);
            return res.status(200).json({successed: true, data: {relationship: rel}});
        }
        else throw new CustomAPIError("Not Permission",403);
    }catch(error:any){
        console.log(error);
...
export const getShareResourceCtl = async (req: Request ,res: Response) => {
    try{        
        const username = getUsername(req, req.params.username);
        let limit = typeof(req.query.limit)=='string'?Number(req.query.limit):10;
...
export const getGroupPostListCtl = async(req: Request, res: Response) =>{
    try{
        ...
        const ok = (typeof(after) == 'string' || typeof(after) == 'undefined') && (typeof(before)=='string'
        ...
        if(username && ok){
            ...
            let latest = '', oldest = '';
            if(posts.length > 0) {
                latest = posts[0].id; oldest = posts[posts.length - 1].id;
            }
            return res.status(200).json({successed: true, data: {
                posts: posts,  latest : latest, oldest : oldest
            }});
        }
